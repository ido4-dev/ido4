...
5. **Don't design solutions.** You're analyzing, not implementing. Note what exists and what's needed — the issue writing agent will decide how to structure the work.
6. **Flag shared infrastructure.** If multiple capabilities need the same foundation (e.g., a shared type, a database table, a service interface), call it out explicitly. This affects how the issue writer groups tasks.
