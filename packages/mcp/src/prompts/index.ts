...
import { z } from 'zod';
import { buildPromptContext, type PromptContext } from './prompt-context.js';
...
- **waveStatus**: active ${container} ${item} breakdown by status
...
- **waveStatus**: ${item} breakdown by status
 ...
 */
function getPrincipleValidationInstructions(principleName: string, ctx: PromptContext): string {
  const container = ctx.containerLabel;
  const items = ctx.itemPlural.toLowerCase();
  const lower = principleName.toLowerCase();

  if (lower.includes('integrity') && lower.includes('epic')) {
    return `- Use the \`epicIntegrityChecks\` array from \`get_compliance_data\` — it contains pre-computed results for every unique epic.
...
export function registerPrompts(server: McpServer, profile: MethodologyProfile): void {
  ...
  const standupPrompt = generateStandupPrompt(ctx);
  const planPrompt = generatePlanContainerPrompt(ctx);
  const boardPrompt = generateBoardPrompt(ctx);
  const compliancePrompt = generateCompliancePrompt(ctx);
  const healthPrompt = generateHealthPrompt(ctx);
  const retroPrompt = generateRetroPrompt(ctx);
